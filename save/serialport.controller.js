              ...
              serialPort.on('open', () => {
                ...
                serialPort.on('data', async (data) => {
                  ...
                  if (dataCount === 2) {
                    const receivedString = receivedData.toString(); // แปลงเป็น string
                    const receivedNumber = parseFloat(receivedString);
                    const floatValue = parseFloat(buffer.toString());
                    const puredata = floatValue;
                ...
                serialPort.on('open', () => {
                  ...
                  serialPort.on('data', async (data) => {
                    ...
                    if (dataCount === 2) {
                      ...
                      io.emit('data', formattedNumber); // ส่งข้อมูลให้ client ผ่าน WebSocket
